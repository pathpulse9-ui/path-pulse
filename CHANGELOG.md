...
- **Root cause of the 2-of-4 treasury:** `buildTreasuryMultisigTx` (`backend/src/stellar/accounts.ts`) hardcoded `masterWeight: 1` in the same `setOptions` that set the thresholds, so the master key stayed a weight-1 signer alongside the three named ones — 4 signers against a threshold of 2. Now sets `masterWeight: 0`. At weight 1 the master could authorize nothing on its own (threshold 2), so it bought no recovery while weakening 2-of-3 into 2-of-4; that is why losing the three signer secrets was fatal rather than merely inconvenient. The function is currently unreferenced by any route.
