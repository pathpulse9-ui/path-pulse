...
The contract in [`packages/contract`](packages/contract) is updated first; clients build against it.

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/health` | Liveness, network and version |
| `GET` | `/v1/accounts/distribution` | Protocol-governed accounts |
| `GET` | `/v1/treasury/config` | Multisig signer set + thresholds |
| `GET` | `/v1/auth/challenge` · `POST` `/v1/auth/wallet/verify` | SEP-10 non-custodial sign-in |
| `POST` | `/v1/auth/google/verify` · `/v1/auth/guest` | Custodial and guest sessions |
| `POST` | `/v1/tx/build` · `/v1/tx/submit` | Delegated build + submit |
| `POST` `GET` | `/v1/settlement/batches` | Execute and list 50 / 30 / 20 batches |
| `GET` | `/v1/settlement/batches/{id}` | Source → Split → Driver drill-down |
| `POST` | `/v1/settlement/group-payouts` | Flat CSV / Excel bulk payout |
| `POST` `GET` | `/v1/ops/payouts/batches` | SDP batch disbursement |
| `POST` `GET` | `/v1/offramp/sessions` | Fiat withdrawal + reconciliation |
| `POST` | `/v1/offramp/callback` | ECDSA-verified provider webhook |
| `GET` | `/v1/scout` · `/v1/scout/{address}` | Issuer config, on-chain tier |
| `POST` | `/v1/scout/assign` | Assign a tier from a validation score |
| `GET` | `/v1/routing/assets` · `/v1/routing/quote` | Routable assets, Aquarius path quote |
| `POST` | `/v1/routing/swap` | Execute a routed swap |
| `GET` | `/v1/gov/settlements` · `/v1/gov/exports` | Partner-scoped audit + CSV / PDF export |
...
pnpm --filter @pathpulse/backend provision:testnet # fund the three distribution accounts
                                                   # paste the printed public keys into .env

pnpm --filter @pathpulse/backend dev               # Backend Core on :8080
pnpm --dir web dev                                 # console on :3000
```

Open `http://localhost:3000/signin`, then head to **Settlement** and run a batch.

**Android** — open [`android/`](android) in Android Studio and run the `app` module. If Gradle can't find a JDK on the command line:
...
```bash
STELLAR_NETWORK=testnet          # testnet | mainnet — mainnet is human-gated
HORIZON_URL=                     # https://horizon-testnet.stellar.org
NETWORK_PASSPHRASE=              # defaults per network

PARTNER_REVENUE_PUBLIC=          # from provision:testnet
DRIVER_POOL_PUBLIC=
TREASURY_PUBLIC=
TREASURY_SIGNER_1_PUBLIC=        # ≥ 2-of-3 threshold signer set
TREASURY_SIGNER_2_PUBLIC=
TREASURY_SIGNER_3_PUBLIC=

SIGNER_BACKEND=dev               # dev | aws-kms | gcp-kms | hsm
SESSION_SECRET=                  # signs the session cookie
SEP10_SIGNING_SECRET=            # signs SEP-10 challenges
GOOGLE_CLIENT_ID=                # Google Identity Services (no OAuth secret needed)

AQUA_ROUTER_CONTRACT=            # Aquarius router contract address
AQUA_API_URL=                    # Aquarius path-finding API
SOROBAN_RPC_URL=
ROUTING_SLIPPAGE_BPS=100         # 100 = 1%

SDP_API_KEY=                     # Stellar Disbursement Platform
SDP_WALLET_ID=
SDP_ASSET_ID=

RAMP_API_KEY=                    # Ramp host key
RAMP_WEBHOOK_PUBLIC_KEY=         # ECDSA public key (PEM) for X-Body-Signature
OFFRAMP_FIAT=INR
```

Every key lives in `.env.example` with a safe default or a blank. `.env`, `.env.*` and `.testnet-accounts.json` are gitignored; only `.env.example` is tracked, and it holds no key material.
